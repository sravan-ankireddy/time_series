...
class TimeMoeInputEmbedding(nn.Module):
    ...
    def __init__(self, config: TimeMoeConfig):
        ...
        self.alt_patch_size = max(1, config.patch_size // 2)  # alternating patch size
        self.hidden_size = config.hidden_size
        
        # Input dimensions for both patch sizes
        patch_input_dim = self.input_size * self.patch_size
        alt_patch_input_dim = self.input_size * self.alt_patch_size
        
        # Embedding layers for primary patch size
        ...
        # Embedding layers for alternating patch size
        self.alt_emb_layer = nn.Linear(alt_patch_input_dim, self.hidden_size, bias=False)
        self.alt_gate_layer = nn.Linear(alt_patch_input_dim, self.hidden_size, bias=False)
    ...
    def forward(self, x):
        ...
        if self.patch_size > 1:
            ...
            while pos < seq_len:
                # Alternate between patch_size and alt_patch_size
                current_patch_size = self.patch_size if patch_idx % 2 == 0 else self.alt_patch_size
                end_pos = min(pos + current_patch_size, seq_len)
                
                # Extract and flatten patch
                patch_data = x[:, pos:end_pos, :]
                if end_pos - pos < current_patch_size:
                    # Pad last patch if needed
                    patch_data = F.pad(patch_data, (0, 0, 0, current_patch_size - (end_pos - pos)))
                
                patch_flat = patch_data.view(batch_size, -1)
                
                # Apply appropriate embedding layers
                if patch_idx % 2 == 0:
                    # Pad to primary patch size if needed
                    if patch_flat.shape[-1] < input_size * self.patch_size:
                        patch_flat = F.pad(patch_flat, (0, input_size * self.patch_size - patch_flat.shape[-1]))
                    emb = self.act_fn(self.gate_layer(patch_flat)) * self.emb_layer(patch_flat)
                else:
                    # Pad to alt patch size if needed
                    if patch_flat.shape[-1] < input_size * self.alt_patch_size:
                        patch_flat = F.pad(patch_flat, (0, input_size * self.alt_patch_size - patch_flat.shape[-1]))
                    emb = self.act_fn(self.alt_gate_layer(patch_flat)) * self.alt_emb_layer(patch_flat)
                
                patch_embeddings.append(emb)
                pos = end_pos
                patch_idx += 1
            
            return torch.stack(patch_embeddings, dim=1)
        else:
            # No patching, direct embedding
...
class TimeMoeOutputLayer(nn.Module):
    ...
    def __init__(self, hidden_size: int, horizon_length: int, input_size: int = 1, patch_size: int = 8):
        ...
        self.alt_patch_size = max(1, patch_size // 2)
        self.out_layer = nn.Linear(hidden_size, input_size * horizon_length, bias=False)
    ...
    def forward(self, x, original_seq_len=None):
        ...
        Returns:
            torch.FloatTensor: prediction with shape [B, seq_len, input_size * horizon_length]
        """
        # Get patch predictions: [B, num_patches, input_size * horizon_length]
        patch_predictions = self.out_layer(x)
        batch_size, num_patches, pred_dim = patch_predictions.shape
        
        # If we only have one patch, return as is
        if num_patches == 1:
            return patch_predictions
        
        # Reshape to separate input_size and horizon_length
        patch_predictions = patch_predictions.view(batch_size, num_patches, self.input_size, self.horizon_length)
        
        # If original_seq_len is not provided, calculate expected sequence length
        # based on alternating patch sizes
        if original_seq_len is None:
            # Calculate expected sequence length from alternating patches
            total_len = 0
            for patch_idx in range(num_patches):
                current_patch_size = self.patch_size if patch_idx % 2 == 0 else self.alt_patch_size
                total_len += current_patch_size
            original_seq_len = total_len
        
        # Create alternating patch size pattern: [patch_size, alt_patch_size, patch_size, ...]
        patch_sizes = torch.tensor([self.patch_size if i % 2 == 0 else self.alt_patch_size 
                                   for i in range(num_patches)], device=x.device)
        
        # Calculate cumulative positions and actual patch sizes
        cumsum_sizes = torch.cumsum(patch_sizes, dim=0)
        remaining_lens = original_seq_len - torch.cat([torch.tensor([0], device=x.device), cumsum_sizes[:-1]])
        actual_patch_sizes = torch.minimum(patch_sizes, remaining_lens)
        actual_patch_sizes = torch.clamp(actual_patch_sizes, min=0)
        
        # Create expanded predictions for all patches at once
        # Shape: [batch_size, num_patches, max_patch_size, input_size, horizon_length]
        max_patch_size = int(actual_patch_sizes.max().item())
        if max_patch_size == 0:
            # Edge case: return empty tensor with correct shape
            return torch.zeros(batch_size, original_seq_len, self.input_size * self.horizon_length, 
                             device=x.device, dtype=patch_predictions.dtype)
        
        # Expand each patch to max_patch_size: [batch_size, num_patches, max_patch_size, input_size, horizon_length]
        expanded_patches = patch_predictions.unsqueeze(2).expand(
            batch_size, num_patches, max_patch_size, self.input_size, self.horizon_length
        )
        
        # Create mask for valid positions within each patch
        patch_positions = torch.arange(max_patch_size, device=x.device).unsqueeze(0)  # [1, max_patch_size]
        valid_mask = patch_positions < actual_patch_sizes.unsqueeze(1)  # [num_patches, max_patch_size]
        
        # Apply mask and flatten: [batch_size, total_valid_positions, input_size, horizon_length]
        valid_positions = expanded_patches[:, valid_mask, :, :]
        
        # Truncate to exact original sequence length
        if valid_positions.shape[1] > original_seq_len:
            valid_positions = valid_positions[:, :original_seq_len, :, :]
        elif valid_positions.shape[1] < original_seq_len:
            # Pad if necessary
            padding_size = original_seq_len - valid_positions.shape[1]
            padding = torch.zeros(batch_size, padding_size, self.input_size, self.horizon_length,
                                device=x.device, dtype=valid_positions.dtype)
            valid_positions = torch.cat([valid_positions, padding], dim=1)
        
        # Reshape to final format: [batch_size, original_seq_len, input_size * horizon_length]
        return valid_positions.view(batch_size, original_seq_len, self.input_size * self.horizon_length)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            for lm_head, horizon_length in zip(self.lm_heads, self.config.horizon_lengths):
                # Pass original sequence length to output layer
                one_predictions = lm_head(hidden_states, original_seq_len=original_seq_len)
            ...
            # Pass original sequence length to output layer
            predictions = lm_head(hidden_states, original_seq_len=original_seq_len)
